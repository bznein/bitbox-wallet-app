...
export const usePaymentReview = ({
  ...
  const preparePayment = useCallback(async (
    ...
    try {
      ...
      if (response.status === 'completed'
        && submittedAttemptKeyRef.current === response.idempotencyKey) {
  ...
  const startNewPayment = useCallback(async () => {
    if (paymentDetails.type !== TPaymentInputType.LNURL_PAY
  ...
  useEffect(() => {
    ...
    return subscribeListPayments((payments) => {
      if (payments.some(payment => payment.id === currentIdempotencyKey)) {
        preparePayment(currentAmountSat, { silent: true });
      }
    });
